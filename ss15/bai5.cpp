#include<stdio.h>
#define V 5 // So dinh toi da cua do thi (co the thay doi tuy bai toan)
...
// Ham DFS kiem tra chu trinh
int hasCycleDFS(int v, int n) {
    // Danh dau dinh hien tai la da tham va dua vao ngan xep de quy
    visited[v] = 1;
    recStack[v] = 1;

    // Duyet qua tat ca cac dinh ke voi dinh hien tai
    for (int i = 0; i < n; i++) {
        // Neu co canh tu dinh v den dinh i
        if (graph[v][i] == 1) {
            // Neu dinh i chua duoc tham, goi de quy
